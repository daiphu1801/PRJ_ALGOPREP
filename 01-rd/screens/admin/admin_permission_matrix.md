...
> Slug: `admin_permission_matrix` — khớp `01-rd/overview/system_survey.md` mục 7.3 [SoT: 01-rd/overview/system_survey.md:555].
> Bounded Context: `identity` (F1) [SoT: 01-rd/overview/system_survey.md:555]. Actor: A3.
>
> Đối chiếu prototype: `09-layoutBase/Admin - Ma trận phân quyền.dc.html`. File này mô tả **hành vi và UX ở
> mức yêu cầu** — không lặp lại đặc tả chức năng đã có ở `01-rd/req/req.md` (F1-10→F1-12), chỉ trỏ tới và bổ
...
[SoT: 01-rd/req/req.md:46-59 — F1-10, F1-11, F1-12].

## 2. Nguồn yêu cầu (không lặp lại — chỉ trỏ)

| Hành vi | Mã | Nguồn |
| :--- | :--- | :--- |
| Ma trận phân quyền theo vai trò, đổi có hiệu lực ngay | F1-10 | `01-rd/req/req.md:46-48` |
| Function/Action là seed cố định chỉ đọc; Role tạo/sửa/xoá được | F1-11 | `01-rd/req/req.md:49-52` |
| Danh sách 11 Function trong phạm vi ma trận | F1-12 | `01-rd/req/req.md:53-59` |
| Given-When-Then đầy đủ | — | `01-rd/req/user_stories.md:284-296` (`US-A3-01`) |
...
1. **Ghi chú đầu trang** — nhắc lại đúng "Lớp 1 — quyền học tập cơ bản không đi qua ma trận" (dòng 163-165)
   — khớp chính xác câu chữ đã chốt ở `req.md:30-34`.
...
4. **Bảng ma trận** — 11 hàng Function (đúng khớp danh sách F1-12), 4 cột Action (Tạo/Xem/Sửa/Xoá), mỗi ô
   là nút toggle vuông (dòng 198-224, 286-298) — khớp hoàn toàn.
5. **Dữ liệu mẫu quyền INSTRUCTOR** — có quyền đầy đủ trên `PROBLEM_AUTHORING`, `TESTCASE_MANAGEMENT`,
   `CLASS_MANAGEMENT`, `INTERVIEW_BANK_MANAGEMENT`; có `C`/`R` nhưng không `U`/`D` trên `REJUDGE_MANAGEMENT`
   (giảng viên tạo/xem được phiên chấm lại nhưng không sửa/xoá) — không có mã `Fx-nn` ghi cụ thể mức quyền
   này, chỉ là dữ liệu seed ví dụ; không phải xung đột (F1-12 chỉ nói phạm vi 11 Function, không nói giá trị
   seed từng ô).
...
- `01-rd/req/req.md:30-34, 46-59` — F1-10, F1-11, F1-12.
- `01-rd/req/user_stories.md:284-296` — `US-A3-01`.
