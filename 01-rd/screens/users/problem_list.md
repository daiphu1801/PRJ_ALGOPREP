...
> cầu** — không lặp lại đặc tả chức năng đã có ở `01-rd/req/req.md` (mục F2) và
> `01-rd/req/user_stories.md` (`US-A1-02`), chỉ trỏ tới và bổ sung phần đặc thù của màn.
...
[SoT: 01-rd/req/req.md:126-130 — F2-11, F2-12].

## 2. Nguồn yêu cầu (không lặp lại — chỉ trỏ)

| Hành vi | Mã | Nguồn |
| :--- | :--- | :--- |
| Tìm kiếm, lọc theo chủ đề/độ khó/trạng thái đã giải | F2-11 | `01-rd/req/req.md:126` |
| Bài giao theo lớp hiển thị thành nhóm riêng trong cùng màn | F2-12 | `01-rd/req/req.md:127-130` |
| Cả hai mô hình nộp bài luôn song song cho mỗi bài toán; học viên tự chọn mỗi lượt làm ở `problem_detail`, trừ khi lược đồ kiểu chưa phủ được thì chỉ còn Standard I/O | F3-13 | `01-rd/req/req.md:179-198` |
| Given-When-Then đầy đủ | — | `01-rd/req/user_stories.md:56-63` (`US-A1-02`) |
...
4. **Khu "Bài tập lớp"** — một khối riêng trong cùng trang (không phải lồng vào từng dòng bảng), liệt kê tên
   ...
   **hiển thị dạng khối tổng hợp cạnh bảng chính, không phải lồng trực tiếp vào từng dòng bài toán** như câu
   chữ ở `req.md:128-129` ("lồng thành một nhóm riêng ngay trong danh sách bài toán") có thể khiến người đọc
...
- **Cho** một bài toán rơi vào ngoại lệ lược đồ kiểu (kiểu dữ liệu vượt lược đồ độc lập ngôn ngữ của F3-01),
  **Khi** tôi xem bài đó trong danh sách, **Thì** nhãn mô hình cho biết bài này **chỉ còn Standard I/O**
  ...
  người học [SoT: 01-rd/req/req.md:187-191 — F3-13 ngoại lệ R2].
- **Cho** một bài toán không rơi vào ngoại lệ trên (đa số các bài), **Khi** tôi xem bài đó trong danh sách,
  **Thì** nhãn mô hình cần thể hiện "cả hai mô hình khả dụng", không còn là một giá trị cố định `function`
  ...
  [SoT: 01-rd/req/req.md:179-186, 192-198 — F3-13 sau `DEC-2026-0824-dual-submission-model-per-problem`;
...
| :-: | :--- | :--- | :--- | :--- |
| Q1 | ~~`req.md:128-129` mô tả bài giao theo lớp "lồng thành một nhóm riêng"...~~ **ĐÃ CHỐT (phiên 2026-08-25, chốt theo RD):** giữ nguyên dạng khối tổng hợp như prototype (đơn giản hơn khi có nhiều lớp/nhiều bài được giao cùng lúc); đã sửa câu chữ `01-rd/req/req.md` mục F2-12 cho khớp thực tế (đổi "lồng thành một nhóm riêng" thành "hiển thị thành một khối riêng ngay trong cùng trang danh sách bài toán"). | — | Đã sửa `req.md`, không cần hành động thêm ở màn này. | Đã đóng |
...
| Q3 | ~~(Mới, phát hiện khi rà Phase 1...) Nhãn "mô hình nộp bài" trong bảng danh sách...~~ **ĐÃ CHỐT (phiên 2026-08-25, chốt theo RD):** đổi nhãn thành "Cả hai" mặc định cho mọi bài có cả hai mô hình song song; chỉ hiện "Chỉ Standard I/O" cho các bài rơi vào ngoại lệ lược đồ kiểu (R2, `req.md:187-191`) — giữ được thông tin hữu ích (biết trước bài nào bị giới hạn) mà không gây hiểu nhầm là chọn cố định. **Chưa dựng vào prototype** (dữ liệu mẫu vẫn gán `mode` cố định 1-trong-2, dòng 501-524) — để lúc build FE Next.js thật. | — | Đã chốt quyết định, chưa dựng prototype. | Đã đóng |
...
- `01-rd/req/req.md:126-130, 179-198` — F2-11, F2-12, F3-13.
...
- `01-rd/req/user_stories.md:56-63` — `US-A1-02`.
